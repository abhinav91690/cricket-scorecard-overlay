...
Rendering is in `src/ui.ts`. `updateScoreboard()` picks team 1 vs team 2 fields based on `values.isSecondInningsStarted === "true"` (API booleans are strings, `isMatchEnded` is `"1"`), then writes through `setText`/`setDisplay` helpers that only touch the DOM when a value changed. The line under the score comes from `statusLine()` (CRR, or Target/Need/RRR computed from the totals); there is no separate second-innings strip any more.
