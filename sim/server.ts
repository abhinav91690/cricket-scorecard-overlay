 ...
 *
 *   node sim/server.ts --port 8788 --speed 60 [--super-over]
 *
...
export function startServer(opts: ServerOptions) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (url.pathname === '/sim/timeline') return json(timeline.map(s => ({ t: s.t, phase: s.phase, t1: s.inn1.total, w1: s.inn1.wickets, t2: s.inn2?.total ?? null, w2: s.inn2?.wickets ?? null, so1: s.so1 ? `${s.so1.total}/${s.so1.wickets}` : null, so2: s.so2 ? `${s.so2.total}/${s.so2.wickets}` : null })));
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    const s = startServer({ port: Number(arg('port', '8788')), speed: Number(arg('speed', '60')), config: { superOver: process.argv.includes('--super-over') } });
